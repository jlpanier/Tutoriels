...
namespace Repository.Dbo
{
    public abstract class BaseDbo
    {
        ...
        private static SQLite.SQLiteConnection _db = null;
        ...
        public static SQLiteConnection Db
        {
            get
            {
                if (_db == null)
                {
                    try
                    {
                        _db = new SQLite.SQLiteConnection(dbPath, SQLiteOpenFlags.ReadWrite | SQLiteOpenFlags.Create | SQLiteOpenFlags.FullMutex, false);
                    }
                    catch (Exception ex)
                    {
                        // Nothing
        ...
        public static bool Init(string databasePath, double busyTimeout = 30)
        {
            try
            {
                dbPath = databasePath;
                Db.BusyTimeout = TimeSpan.FromSeconds(busyTimeout);
                Db.CreateTable(typeof(Country));
                Db.DropTable<NombrePremierEntity>();
                Db.CreateTable(typeof(NombrePremierEntity));
                return true;
            }
            catch
            {
